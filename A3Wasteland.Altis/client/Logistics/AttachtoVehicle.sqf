...
private _displayname = str configProperties [configFile>>"cfgVehicles">>_class>>"displayName"];
hint _displayname;
...
if (_distance <=10) then
{
	switch (true) do 
	{
		...
		case (_vehicle iskindof "Offroad_01_base_F"): 
		{
			...
			private _attachedStatic = _vehicle getVariable ["StaticAttached", false];		// Special Spot for Static Weapon (may preclude loading of static objects)
			private _attachedSLD = _vehicle getVariable ["LocationSLD_Attached", false];	// Special Spot for Static laser designator
			private _Attached1 = _vehicle getVariable ["Location1_Attached", false];		// Used for Vehicle Ammo Crate
			private _Attached2 = _vehicle getVariable ["Location2_Attached", false];		// Used for Supply crates
			private _Attached3 = _vehicle getVariable ["Location3_Attached", false];		// Used for large crates
			private _attached4 = _vehicle getVariable ["Location4_Attached", false];		// Used for speacial weapons crate Right side
			private _attached5 = _vehicle getVariable ["Location5_Attached", false];		// Used for speacial weapons crate Left Side
			private _attached6 = _vehicle getVariable ["Location6_Attached", false];		
			private _attached7 = _vehicle getVariable ["Location7_Attached", false];
			private _attached8 = _vehicle getVariable ["Location8_Attached", false];
			private _attached9 = _vehicle getVariable ["Location9_Attached", false];
			private _attached10 = _vehicle getVariable ["Location10_Attached", false];

			//Define Attachment points
			private _attach1 = [1.49012e-008,-1.8,0.3];
			private _attach2 = [1.49012e-008,-1.8,0.1];
			private _attach3 = [1.49012e-008,-1.8,-0.3];
			private _attach4 = [0.3,-1.8,-0.5];
			private _attach5 = [-0.4,-1.8,-0.5];
			private _attach6 = [0,-1.1,-0.4];
			private _atach7 = [0,-1.7,-0.4];
			private _attach8 = [0,-2.3,-0.4];

			//make sure theres an empty spot
			private _spotarray = [_attachedStatic,_Attached1,_Attached2,_Attached3,_attached4,_attached5,_attached6,_attached7,_attached8,_attached9,_attached10,_attachedSLD];
			private _spotcheck = [false];
			private _spots = _spotarray arrayIntersect _spotcheck;
