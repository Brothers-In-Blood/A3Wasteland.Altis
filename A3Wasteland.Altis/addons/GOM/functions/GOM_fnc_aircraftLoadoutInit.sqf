...
GOM_fnc_aircraftLoadout_NeedsFuelSource = true; //(default: true) needs fuel supply within 50m of the Vehicle or functions will be unavailable
GOM_fnc_aircraftLoadout_NeedsAmmoSource = true; //(default: true) needs ammo supply within 50m of the Vehicle or functions will be unavailable
...
GOM_fnc_setPylonsRearm =
{
	...
	_nul = [_obj,_rearm,_pylons,_pylonAmmoCounts] spawn
	{
		...
		if (_veh getVariable ["GOM_fnc_aircraftLoadoutRearmingInProgress",false]) exitWith {systemchat "Vehicle is already currently being rearmed!"};
...
GOM_fnc_setPylonsRepair =
{
	...
	_refuel = [_veh,_repairSource,_obj] spawn
	{
		...
		if (_curDamage isEqualTo 1) exitWith {systemchat "Vehicle is destroyed!"};
		...
		systemchat "Vehicle has to remain stationary during repair procedure!";
...
GOM_fnc_setPylonsRefuel =
{
	...
	_refuel = [_veh,_refuelSource,_obj] spawn
	{
		...
		_curFuel = fuel _veh;
		if (_curFuel isEqualTo 1) exitWith {systemchat "Vhicle is already at 100% fuel capacity!"};
		_maxFuel = getNumber (configfile >> "CfgVehicles" >> typeof _veh >> "fuelCapacity");
		...
		systemchat "Vehicle has to remain stationary during refuelling procedure!";
...
GOM_fnc_aircraftLoadoutResourcesCheck =
{
	...
	{
		_flags set [0,!GOM_fnc_aircraftLoadout_NeedsFuelSource]
	};
	if (GOM_fnc_aircraftLoadout_NeedsAmmoSource) then
	{
	...
	{
		_flags set [2,!GOM_fnc_aircraftLoadout_NeedsAmmoSource]
	};
	if (GOM_fnc_aircraftLoadout_NeedsRepairSource) then
	{
	...
	{
		_flags set [1,!GOM_fnc_aircraftLoadout_NeedsRepairSource]
	};
	_flags params ["_canRefuel","_canRepair","_canRearm"];
...
GOM_fnc_aircraftLoadout =
{
	...
	(finddisplay 66 displayctrl 1100) ctrlSetStructuredText parsetext "<t align='center'>Select a Vehicle!";
	...
	finddisplay 66 displayCtrl 1500 ctrlAddEventHandler ["LBSelChanged",format ["[%1] call GOM_fnc_setPylonLoadoutLBPylonsUpdate; [%1] call GOM_fnc_updateDialog;[%1] call GOM_fnc_aircraftLoadoutPaintjob;",_getvar]];
	finddisplay 66 displayCtrl 1501 ctrlAddEventHandler ["LBSelChanged",format ["[%1] call GOM_fnc_fillPylonsLB; [%1] call GOM_fnc_updateDialog",_getvar]];
	finddisplay 66 displayCtrl 1502 ctrlAddEventHandler ["LBSelChanged",format ["[%1] call GOM_fnc_setPylonLoadoutLBMagazinesCountUpdate; [%1] call GOM_fnc_updateDialog",_getvar]];
