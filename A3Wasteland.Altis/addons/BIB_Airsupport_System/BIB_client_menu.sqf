...
[
	["Airdrop",true],
			...
			["Air Support", [4], "USER:AirSupporMenu", -5, [["expression", ""], "1", "1"]],
...
/////////////////////////////////////////////////////
AirSupportMenu = [];
_startAirSupMenu = ["Air Support",true];
Menu pushback _startAirSupMenu;

_i=0;
{
	_optionAirSupMenu = [];
	_lineElement1=format ["%1 ($%2)",(APOC_AA_AirSupOptions select _i) select 0, (APOC_AA_SupOptions select _i) select 2];
	_type = (APOC_AA_AirSupOptions select _i) select 3;
	
	_optionAirSupMenu pushback _lineElement1;
	_optionAirSupMenu append [[_i+2], "", -5];
	_optionAirSupMenu pushback [["expression", format ['["%1",%2,player] execVM "addons\APOC_Airdrop_Assistance\APOC_cli_startAirdrop.sqf"',_type,_i]]];
	_optionAirSupMenu append ["1","1"];

	SupplyMenu pushback _optionAirSupMenu;
	//diag_log format["Here's the menu structure: %1",SupplyMenu];
	_i=_i+1;
}forEach APOC_AA_AirSupOptions;

_endAirSupMenu = ["Cancel Airdrop", [_i+2], "", -3, [["expression", ""]], "1", "1"];
AirSupportMenu pushback _endAirSupMenu;
///////////////////////////////////////////////////////
