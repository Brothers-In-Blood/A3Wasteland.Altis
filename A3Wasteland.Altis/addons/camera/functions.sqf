...
camera_keyUpHandler = 
{
	...
	if ((_key in (actionKeys "MoveForward") || _key in (actionKeys "MoveBack") || _key in (actionKeys "TurnLeft") || _key in (actionKeys "TurnRight") || _key in [DIK_Q, DIK_Z]) && (count(camera_key_tracker) == 0)) then 
	{
...
camera_update_target = 
{
	...
	if (_shift && _key in (actionKeys "Chat")) then 
	{
		...
		{ if (not(_aiming_at_same_player) && {not(isNil "_new_target")}) then 
			{
