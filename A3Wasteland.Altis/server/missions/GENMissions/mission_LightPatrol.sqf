...
_setupObjects =
{
	...
	_createVehicle = 
	{
		...
		if (_drivers > 0) then
		{
			...
			{
				private _Driver = [_aiGroup, _position, "GEN", "Rifleman"] call createsoldier;
		...
		if (_Commanders > 0) then
		{
			...
			{
				private _Commander = [_aiGroup, _position, "GEN", "Rifleman"] call createsoldier;
				_Commander moveInCommander _vehicle;
			};
		};
		if (_Gunners > 0) then
		{
			private _gunner = [_aiGroup, _position, "GEN", "Rifleman"] call createsoldier;
		...
		if (_Passangers > 0) then
		{
			...
			{
				_soldier = [_aiGroup, _position, "GEN", "Rifleman"] call createsoldier;
	...
	_missionHintText = format ["A convoy containing at least a <t color='%2'>%1</t> is patrolling Altis! Stop the patrol and capture the goods and money!", _vehicleName, GENMissionColor];
