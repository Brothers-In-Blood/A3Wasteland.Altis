...
_setupObjects =
{
	...
	_vehicles = [_veh1,_veh2,_veh3];
	{
		...
		if (_drivers > 0) then
		{
			...
			{
				private _Driver = [_aiGroup1, _missionPos, "CSAT", "CViperrew"] call createsoldier;
		...
		if (_Commanders > 0) then
		{
			...
			{
				private _Commander = [_aiGroup1, _missionPos, "CSAT", "Viper"] call createsoldier;
				_Commander moveInCommander _vehicle;
			};
		};
		if (_Gunners > 0) then
		{
			private _gunner = [_aiGroup1, _missionPos, "CSAT", "Viper"] call createsoldier;
		...
		if (_Passangers > 0) then
		{
			...
			{
				_soldier = [_aiGroup1, _missionPos, "CSAT","Viper"] call createsoldier;
	...
	_vehicleName2 = getText (configFile >> "CfgVehicles" >> _veh2types >> "displayName");
	_vehicleName3 = getText (configFile >> "CfgVehicles" >> _veh3types >> "displayName");
