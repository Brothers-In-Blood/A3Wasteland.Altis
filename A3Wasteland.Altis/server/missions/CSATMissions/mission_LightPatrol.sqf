...
_setupObjects =
{
	...
	createMissionVehicle = 
	{
		...
		if (_drivers > 0) then
		{
			...
			{
				private _Driver = [_aiGroup1, _position, "CSAT", "Crew"] call createsoldier;
		...
		if (_Commanders > 0) then
		{
			...
			{
				private _Commander = [_aiGroup1, _position, "CSAT", "Crew"] call createsoldier;
				_Commander moveInCommander _vehicle;
			};
		};
		if (_Gunners > 0) then
		{
			private _gunner = [_aiGroup1, _position, "CSAT", "Crew"] call createsoldier;
	...
	_missionHintText = format ["A convoy containing at least a <t color='%4'>%1</t> and a <t color='%4'>%2</t> is patrolling Altis! Stop the patrol and capture the goods and money!", _vehicleName, _vehicleName2, CSATMissionColor];
