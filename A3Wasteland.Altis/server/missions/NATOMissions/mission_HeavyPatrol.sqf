...
_setupObjects =
{
	...
	_createVehicle = 
	{
		...
		if (_drivers > 0) then
		{
			...
			{
				private _Driver = [_aiGroup, _position, "NATO", "Crew"] call createsoldier;
		...
		if (_Commanders > 0) then
		{
			...
			{
				private _Commander = [_aiGroup, _position, "NATO", "Crew"] call createsoldier;
				_Commander moveInCommander _vehicle;
			};
		};
		if (_Gunners > 0) then
		{
			private _gunner = [_aiGroup, _position, "NATO", "Crew"] call createsoldier;
