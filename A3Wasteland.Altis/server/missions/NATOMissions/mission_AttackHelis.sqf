...
_setupObjects =
{
	...
	_veh1 = selectrandom ["B_Heli_Light_01_dynamicLoadout_F","B_Heli_Attack_01_dynamicLoadout_F"];
	_veh2 = selectrandom ["B_Heli_Light_01_dynamicLoadout_F","B_Heli_Attack_01_dynamicLoadout_F"];
	_createVehicle = 
	{
		...
		if (_drivers > 0) then
		{
			...
			{
				private _Driver = [_aiGroup, _position, "NATO", "HeliPilot"] call createsoldier;
				_Driver moveInDriver _vehicle;
			};
		};
		private _Copilot = [_aiGroup, _position, "NATO", "HeliPilot"] call createsoldier;
		_Copilot moveInAny _vehicle;
		if (_Gunners > 0) then
		{
			private _gunner = [_aiGroup, _position, "NATO", "HeliCrew"] call createsoldier;
		...
		if (_Passangers > 0) then
		{
			for "_i" from 1 to _Passangers do
			{
				...
				_soldier = [_aiGroup, _missionPos, "NATO", _soldierType] call createsoldier;
	...
	_missionHintText = format ["A convoy containing at least a <t color='%3'>%1</t> and a <t color='%3'>%2</t> is patrolling Altis! Stop the patrol and capture the goods and money!", _vehicleName, _vehicleName2, NATOMissionColor];
