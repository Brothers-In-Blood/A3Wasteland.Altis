...
_setupVars =
{
	_missionType = "SYN Medium Convoy";
...
_setupObjects =
{
	...
	_vehicles = [_veh1,_veh2,_veh3,_veh4,_veh5];
	{
		...
		if (_drivers > 0) then
		{
			...
			{
				private _Driver = [_aiGroup1, _missionPos, "SYN", "Crew"] call createsoldier;
		...
		if (_Commanders > 0) then
		{
			...
			{
				private _Commander = [_aiGroup1, _missionPos, "SYN", "Crew"] call createsoldier;
				_Commander moveInCommander _vehicle;
			};
		};
		if (_Gunners > 0) then
		{
			private _gunner = [_aiGroup1, _missionPos, "SYN", "Crew"] call createsoldier;
