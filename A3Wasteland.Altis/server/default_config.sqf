...
// Time settings
A3W_startHour = 07;							// In-game hour at mission start (0 to 23) - time is saved and restored between server restarts if A3W_timeSaving = 1
...
A3W_remoteBombStoreRadius = 20;    // Prevent players from placing any kind of explosive on the ground within this distance from any store (0 = disabled)
A3W_poiObjLockDistance = 20;      // Prevent players from locking objects within this distance from points of interest (stores & mission spawns)
...
A3W_privateParkingCost = 25000;		// Cost to retrieve an individual vehicle from private parking
...
A3W_extDB_Environment = "AtlisLive1";  // Value used to separate player & object data from multiple environments running on the same map (e.g. "normal", "hardcore", "dev", etc. can be whatever you want)
...
A3W_payrollInterval = 1;			// Delay in seconds between each payroll
...
A3W_PrimaryMissionDelay = 240*60;		//Time in seconds before Primary Missions start
