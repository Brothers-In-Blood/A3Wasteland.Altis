...
A3W_teamBalance = 1;               // Enable team balancing between OPFOR and BLUFOR
...
PDB_ObjectFileID = "A3WD_";         // Object savefile prefix (if you run multiple servers, change it to a unique value for each server)
...
A3W_extDB_Environment = "Altis Live 1";  // Value used to separate player & object data from multiple environments running on the same map (e.g. "normal", "hardcore", "dev", etc. can be whatever you want)
...
A3W_PrimaryMissionDelay = 240*60;
A3W_PrimaryMissionTimeout = 240*60;
